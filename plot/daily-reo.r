...
days[ , year := as.integer(str_match(date, '(18|19|20)[0-9]{2}')[, 1])]
...
years <- merge(years, reo_by_year, by='year', all.x=T)

g <- ggplot(years, aes(x=year, y = reo_speeches/(reo+other+ambiguous))) + 
     geom_bar(fill='#DB2850', stat='identity', width=1) + 
     scale_x_continuous(breaks=seq(1860, 2010, by=10), expand=c(0, 0), limits=c(1850, 2020)) +
     scale_y_continuous(labels=percent,expand=c(0,0), limits=c(-0.0002, 0.02)) +
     theme_bw() +
     ylab('Te Reo spoken in parliament') +
     xlab('Year') +
